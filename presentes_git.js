...
//Mensagem de confirmação de presença via WhatsApp
function confirmarPresenca() {
  // Capturar valores dos campos
  const nome = document.getElementById("nomeform").value.trim();
  const qtdPessoas = document.getElementById("qtdpessoas").value.trim();
  const presenca = document.getElementById("presenca").value;

  // Validar se os campos foram preenchidos
  if (nome === "" || nome.value === String) {
    mostrarToast("❌ Por favor, insira seu nome!", 'erro');
    return false;
  }

  // Construir a mensagem com os dados do formulário
  const tx = [
    "> *Notificação automática: Confirmação de Presença para o Casamento*",
    "",
    `${presenca === "true" ? "Estou confirmado(a)!" : "Sinto muito! Não poderei comparecer ao seu casamento."}`,
    "",
    `*Nome:* ${nome}`,
    `*Quantidade de pessoas:* ${qtdPessoas == 0 || "" ? "Somente eu." : qtdPessoas + " acompanhante(s)."}`,
    "",
    "Estou muito feliz por vocês e desejo toda a felicidade do mundo!"
  ].join("\n");
  
  return `https://wa.me/${whatsappNoivo}?text=${encodeURIComponent(tx)}`;
}
...
function gerarLinkWhatsappPagamento(presente) {
  const tx = [
    "> *Notificação automática: Quero presentear vocês!*",
    "",
...
const presentes = [
  ...
  { id: 25, nome: 'Jogo de Cama Queen', valor: 158, imagem: './25.png', categoria: 'quarto', linkpgm: '#', pago: true, pagoPor: 'Célia Silva Cruz'},
